 ...
 /*  const searchMedications = async (searchTerm) => {
  ...
  const searchMedications = async () => {
    try {
      ...
      if (response.data) {
        console.log('Medicamentos encontrados:', response.data);
        setMedications(response.data); // Atualiza o estado
        setError(''); // Limpa possíveis mensagens de erro
      }
    } catch (error) {
      console.error('Erro ao buscar medicamentos:', error);
 ...
 */
  const selectMedication = async (medication) => {
    ...
    try {
      console.log('ID do medicamento enviado para a API:', medication.id_medicamento);
      const response = await axios.get(`/api/getMedicamentoById`, {
        params: { id: medication.id_medicamento },
      });
      ...
      if (response.data) {
        ...
        setDates({
          ...
          nometrat: updatedMedication.nome_tratamento || '',
          nomemedico: updatedMedication.nome_medico || '',
          isEntregaDisabled: updatedMedication.pago !== 'PG',
        });
      } else {
        console.error('Nenhum dado retornado para o ID do medicamento:', medication.id_medicamento);
        setError('Nenhum dado encontrado para o medicamento selecionado.');
