...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    const { id_paciente, id_medico, obs, dtconsulta, id_usuario, gerarReceita, receitaTexto, gerarExame, exameTexto } = req.body;
    ...
    try {
      ...
      if (gerarReceita && receitaTexto) {
        const queryReceituarios = `
          INSERT INTO receituarios (id_fichamedica, receita, tipo, dtconsulta, id_usuario)
          VALUES (?, ?, ?, ?, ?)
        `;
      ...
      if (gerarExame && exameTexto) {
        const queryExames = `
          INSERT INTO exames (id_fichamedica, nome_exame, tipo, dtconsulta, id_usuario)
          VALUES (?, ?, ?, ?, ?)
        `;
        const paramsExames = [
          id_fichamedica,
