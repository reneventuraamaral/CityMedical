...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    ...
    try {
      ...
      const query = 'INSERT INTO tratamentos (nome, dtcad, duracao, id_usuario) VALUES (?, ?, ?, ?)';
      const values = [nome, dtcad, duracao, id_usuario];
      ...
      res.status(201).json({ message: 'Tratamento cadastrado com sucesso!' });
    } catch (error) {
